...
# Usage: `bash ./load_observations_station.sh path/to/file.txt item datetime`
#
# TODO:
# - credential handling
...
# Arguments
file=${1}
item=${2:-"tageswerte_TST"}
datetime=${3:-"$(date --utc +%Y-%m-%dT%H:%M:%SZ)"}

mediatype="application/json"
collection="ch.meteoschweiz.tageswerte"

# Asset key for S3
key="$collection/$item/$(basename "$file")"

# Asset id = file name without extention
